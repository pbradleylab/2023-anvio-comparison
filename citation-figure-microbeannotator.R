...
calculateNodeSizes <- function(edges) {
  # Create a graph object
  graph <- graph_from_data_frame(edges, directed = TRUE)
  
  # Calculate node sizes based on the number of edges
  node_sizes <- sapply(V(graph), function(node) degree(graph, node))
  return(node_sizes)
}
...
)  %>% htmlwidgets::prependContent(htmltools::tags$h1("Downstream Citation Tree of Original Publication")) %>% 
  saveNetwork(file = 'citation_tree_annon.html')
