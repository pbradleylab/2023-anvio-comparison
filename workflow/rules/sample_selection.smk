...
rule gunzip_gff:
    input:rules.download_gff.output
    output:"resources/genomes/gff/{genome}.gff"
    conda:"../envs/sample_selection.yml"
    shell:
        """
        gunzip -c {input} > {output}
...
rule gffread:
    input:
        ref=rules.gunzip.output,
        gff=rules.gunzip_gff.output
    output:"resources/genomes/gffread/{genome}_cds.frn"
    conda:"../envs/sample_selection.yml"
    shell:
        """
        gffread -w {output} -g {input.ref} {input.gff}
        """

rule transeq:
    input:rules.gunzip.output
    output:"resources/genomes/transeq/{genome}/{genome}.faa",
    conda:"../envs/sample_selection.yml"
    shell:
        """
...
rule transeq_cds:
    ...
    output:"resources/genomes/transeq/{genome}/{genome}_cds.faa",
    conda:"../envs/sample_selection.yml"
    shell:
        """
        transeq {input} {output} 
